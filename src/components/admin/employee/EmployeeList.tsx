...
interface Employee {
  ...
  department: string;
  designation: string;
  employeeId: string; // You might want to generate this
  isActive: boolean;
  createdAt: string;
  profileImage?: string;
  addedBy?: string;
    ...
    const fetchEmployees = onValue(employeesRef, (snapshot) => {
      ...
      snapshot.forEach((childSnapshot) => {
        ...
        employeesData.push({
          ...
          employeeId: employee.employeeId || `EMP-${childSnapshot.key?.slice(0, 8)}` // Generate ID if not exists
        });
      });

      setEmployees(employeesData);
      setFilteredEmployees(employeesData); // Initial filter shows all
      setLoading(false);
    }, (error) => {
